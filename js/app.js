  ...
  if (shipLength === 5) {
    if (player.ships.battleship.pegs.every((node) => node === null)) {
      ...
      userPrompt.innerHTML =
        "Select an adjacent space left, right, up, or down to place your Battleship in selected direction";
    } else if (
      firstNode + 10 !== selectedNode &&
      firstNode - 10 !== selectedNode &&
      firstNode + 1 !== selectedNode &&
      firstNode - 1 !== selectedNode
    ) {
      userPrompt.innerHTML = "Not a valid selection";
    } else if (
      selectedNode === selectedNode + 10 ||
      selectedNode - 10 ||
      selectedNode + 1 ||
      selectedNode - 1
    ) {
      ...
      if (selectedNode === firstNode + 10) {
        ...
        document.getElementById(selectedNode + 10).style =
          "background-color: orchid";
        player.ships.battleship.pegs[3] = selectedNode + 20;
        document.getElementById(selectedNode + 20).style =
          "background-color: orchid";
        player.ships.battleship.pegs[4] = selectedNode + 30;
        document.getElementById(selectedNode + 30).style =
          "background-color: orchid";
        ...
        document.getElementById(selectedNode + 1).style =
          "background-color: orchid";
        player.ships.battleship.pegs[3] = selectedNode + 2;
        document.getElementById(selectedNode + 2).style =
          "background-color: orchid";
        player.ships.battleship.pegs[4] = selectedNode + 3;
        document.getElementById(selectedNode + 3).style =
          "background-color: orchid";
      } else if (selectedNode === firstNode - 1) {
        player.ships.battleship.pegs[2] = selectedNode - 1;
        document.getElementById(selectedNode - 1).style =
    ...
    if (player.ships.cruiser.pegs.every((node) => node === null)) {
      ...
      userPrompt.innerHTML =
        "Select an adjacent space left, right, up, or down to place your Cruiser in selected direction";
    } else if (
      firstNode + 10 !== selectedNode &&
      firstNode - 10 !== selectedNode &&
      firstNode + 1 !== selectedNode &&
    ...
    if (player.ships.sub.pegs.every((node) => node === null)) {
      ...
      userPrompt.innerHTML =
        "Select an adjacent space left, right, up, or down to place your Sub in selected direction";
    } else if (
      firstNode + 10 !== selectedNode &&
      firstNode - 10 !== selectedNode &&
      firstNode + 1 !== selectedNode &&
...
function cpuPlace() {
  ...
  do {
    ...
    if (
      !cpu.ships.destroyer.every((node) => {
        return computerBoard.includes(node * -1);
      })
    ) {
      check === 1;
...
function checkSunk(guess) {
  if (player.ships.battleship.pegs.includes(guess)) {
    if (
      player.ships.battleship.pegs.every((node) => gameBoard[node - 1] === -10)
    ) {
      player.ships.battleship.pegs.forEach(
        (node) => gameBoard[node - 1] === "sunk"
      );
    ...
    if (
      player.ships.cruiser.pegs.every((node) => gameBoard[node - 1] === -10)
    ) {
      player.ships.cruiser.pegs.forEach(
        (node) => gameBoard[node - 1] === "sunk"
      );
    ...
    if (
      player.ships.destroyer.pegs.every((node) => gameBoard[node - 1] === -10)
    ) {
      player.ships.destroyer.pegs.forEach(
        (node) => gameBoard[node - 1] === "sunk"
      );
...
function checkCpuSunk(guess) {
  if (cpu.ships.battleship.includes(guess * -1)) {
    if (
      cpu.ships.battleship.every(
        (node) => computerBoard[node * -1 - 1] === "hit"
      )
    ) {
    ...
    if (
      cpu.ships.cruiser.every((node) => computerBoard[node * -1 - 1] === "hit")
    ) {
  ...
  do {
    if (Guess.prevHits.length < 1) {
      ...
      if (Guess.cpuGuessDirection === 1) {
        guess = guess - 10;
        while (
          Guess.cpuGuesses.includes(gameBoard[guess - 1]) &&
          hitWasGuessed !== 1
        ) {
          guess = guess - 10;
        }
      ...
      if (
        Guess.prevDirection.direction === 1 ||
        Guess.prevDirection.direction === 3
      ) {
        // We know its Vertical ----------------------------------------------------
        vertGuess = getRandomIntInclusive(1, 2);
        if (vertGuess === 1) {
          // GUESS UP WITHIN BOUNDS AND NOT IN CPUGUESSES
          ...
          // GUESS DOWN WITHIN BOUNDS AND NOT IN CPUGUESSES
          guess = Guess.prevHits[0] + 10;
          while (gameBoard[guess - 1] === -10 && hitWasGuessed !== 1) {
            guess = guess + 10;
          }
        ...
        Guess.prevDirection.direction === 2 ||
        Guess.prevDirection.direction === 4
      ) {
        // We know its horizontal ------------------------------------------
        horizGuess = getRandomIntInclusive(1, 2);
        if (horizGuess === 1) {
          // GUESS RIGHT WITHIN BOUNDS AND NOT IN CPU GUESSES
        ...
        } else if (horizGuess === 2) {
          // GUESS LEFT WITHIN BOUNDS AND NOT IN CPU GUESSES
...
function playerSwap() {
  if (currentTurn === "cpu") {
    ...
    currentTurn = "cpu";
    userPrompt.innerHTML =
      "CPU's TURN! PREPARE TO BE FIRED ON IN 3... 2... 1... ";
  }
}
